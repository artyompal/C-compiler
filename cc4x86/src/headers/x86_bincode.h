...
typedef enum x86_operand_location_decl {
    ...
    x86loc_int_constant,    // операнд - абсолютна€ константа
    x86loc_symbol,          // операнд - адрес с относительным смещением
    x86loc_symbol_offset,   // операнд - относительна€ константа
...
typedef struct x86_operand_decl {
    ...
    union data_decl {
        ...
        // 0 - не используетс€, >0 - псевдо-регистр, <0 - реальный регистр
        int         reg;
...
typedef enum x86_instruction_code_decl {
    ...
    // »нструкции дл€ внутреннего пользовани€.
    ...
    x86insn_rep_movsb,          // параметры - псевдорегистры дл€ EDI,ESI,ECX, все измен€ютс€
    x86insn_rep_movsd,          // параметры - псевдорегистры дл€ EDI,ESI,ECX, все измен€ютс€
...
#define OP_IS_SYM_OFFSET(OP)            ((OP).op_loc == x86loc_symbol_offset)
#define OP_IS_SYMBOL_OR_OFFSET(OP)      ((OP).op_loc == x86loc_symbol || (OP).op_loc == x86loc_symbol_offset)
#define OP_IS_CONSTANT_OR_OFFSET(OP)    ((OP).op_loc == x86loc_int_constant || (OP).op_loc == x86loc_symbol_offset)
...
// ¬се следующие функции вставл€ют инструкцию перед заданной.
