...
void unit_push_expression(expression *expr)
{
    ...
    // вставка элемента в двусв€зный список
...
void unit_insert_slice(expression *list)
{
    ...
    // вставка подсписка в двусв€зный список
    if (!_curr_func->func_body) {
...
//
//  ”даление выражений, бессмысленных с точки зрени€ кодогенерации.
//
...
//
//  ¬ыделение логической арифметики внутри условных переходов.
//  ћы помечаем дл€ кодогенератора такие команды специальными кодами.
//
...
//
//  «амен€ем логические операции, не подпавшие под предыдущие проверки,
...
//
//  ¬ычисление метрики сложности дл€ всех выражений.
//
...
//
//  ”даление переменных, вышедших из области видимости, из таблицы символов.
//
...
//
//  ќбработка деклараций переменных и функций.
//
...
void unit_handle_function_body(symbol *sym)
{
    ...
    // —начала выкидываем бессмысленные, с точки зрени€ кодогенератора, выражени€.
    _eliminate_meaningless_expressions();

    // «амен€ем операторы &&, || внутри jmp на специальные коды op_logical_and_in_jump/op_logical_or_in_jump.
    _process_boolean_operations_in_jumps();

    // «амен€ем логическую арифметику (операторы &&, ||) на условные переходы с записью результата
    // во временную переменную.
    _process_general_boolean_arithmetic();

    // ¬ычисл€ем метрику сложности дл€ каждого выражени€, в том числе дл€ вложенных.
    _calc_expressions_complexity();

    // ”дал€ем параметры функции и локальные переменные из области видимости.
    // TODO: поддержка локальных переменных во вложенных блоках с правильными област€ми видимости.
    _remove_function_params_from_symtable(sym->sym_type);
