...
expression *expr_create_unary(expression *expr, arithmetic_opcode opcode)
{
    ...
    if (IS_INTERNAL(opcode)) {
        // у инструкций, сгенерированных компил€тором, всЄ должно быть корректно
    ...
    // создаЄм результирующее выражение
    ...
    // вычисл€ем константные выражени€
...
static expression *_create_binary_arithm_expr(arithmetic_opcode opcode, expression *e1, expression *e2, data_type *type)
{
    ...
    // если выражение константное, тривиально вычисл€ем его
    ...
    // замен€ем умножение/деление на константную степень двойки битовыми сдвигами
...
expression *expr_create_binary(expression *e1, expression *e2, arithmetic_opcode opcode)
{
    ...
    // провер€ем допустимость типов операндов
    ...
    // провер€ем случай сравнени€ указателей
    if (TYPE_IS_POINTER(e1->expr_type)) {
    ...
    // провер€ем присваивание структур
    ...
    // дл€ присваивани€ первый операнд об€зан быть lvalue
    ...
    // выводим тип результата; результат сравнени€ всегда int.
    if (IS_COMPARE_OP(opcode)) {
...
expression *expr_create_function_call(expression *address, expression_list *args)
{
    ...
    // провер€ем тип выражени€ с адресом функции
    ...
    // провер€ем типы аргументов и делаем необходимые преобразовани€ типов
...
// »тератор дл€ рекурсивного обхода подвыражений.
