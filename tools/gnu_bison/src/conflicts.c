  ...
  if (fixed_outfiles)
    {
	 ...
	 specified by POSIX.  */
      printf( _("conflicts: "));
      if (src_total > 0)
	printf( _(" %d shift/reduce"), src_total);
      if (src_total > 0 && rrc_total > 0)
	printf( ",");
      if (rrc_total > 0)
	printf( _(" %d reduce/reduce"), rrc_total);
      putc('\n', stdout);
    }
  else
    {
      printf( _("%s contains"), infile);

      if (src_total == 1)
	printf( _(" 1 shift/reduce conflict"));
      else if (src_total > 1)
	printf( _(" %d shift/reduce conflicts"), src_total);

      if (src_total > 0 && rrc_total > 0)
	printf( _(" and"));

      if (rrc_total == 1)
	printf( _(" 1 reduce/reduce conflict"));
      else if (rrc_total > 1)
	printf( _(" %d reduce/reduce conflicts"), rrc_total);
