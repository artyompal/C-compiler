...
{
  if (infile == 0)
    printf( _("%s: fatal error: "), program_name);
  else
    printf( _("%s:%d: fatal error: "), infile, lineno);
}
...
{
  if (infile == 0)
    printf( _("%s: "), program_name);
  else
    printf( _("%s:%d: "), infile, lineno);
  failure = 1;
}
...
{
  fatal_banner ();
  printf( _("too many %s (max %d)"), s, MAXSHORT);
  fputc ('\n', stdout);
  done (1);
}
