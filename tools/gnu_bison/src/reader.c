 ...
 * which contains a switch statement to decide which guard or action
 * to execute.
 *
...
void reader(void)
{
...
#if 0
   translations   = 0;     /* initially assume token number       *
    ...
    * and fdefines file. Also notice any %token, %left, etc. found there.
    */
   if (noparserflag)
...
//   fprintf(ftable, "#include \"stdafx.h\"\n\n\n");
   fprintf(ftable, "\n\n");
   ...
   /*
    * read in the grammar, build grammar in list form.
    * Write out guards and actions.
    */
   readgram();
    ...
    */
   if (fdefines) reader_output_yylsp(fdefines);

   /* write closing delimiters for actions and guards.  */
   output_trailers();

   if (yylsp_needed) fprintf(ftable, "#define YYLSP_NEEDED\n\n");
